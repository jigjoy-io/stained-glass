import LocalizedStrings from "react-localization"

const localization = new LocalizedStrings({
	US: {
		profile: "Profile",
		profileDescription: "Showcase personal information and expertise.",
		blankPage: "Blank Page",
		blankPageDescription: "Create a sub-page inside this page",
		carousel: "Carousel",
		carouselDescription: "Split content into multiple connected pages.",
		text: "Text",
		textDescription: "Begin writing with plain text",
		heading: "Heading",
		headingDescription: "Create a medium section heading",
		title: "Title",
		titleDescription: "Create a bold section heading",
		image: "Image",
		imageDescription: "Upload an image or insert its link.",
		video: "Reel",
		videoDescription: "Upload an video or insert its link.",
		audio: "Audio",
		audioDescription: "Upload audio or insert its link.",
		question: "Question",
		questionDescription: "Test user's knowledge by asking question.",
		message: "Message",
		messageDescription: "Display message bubble.",
	},
	RS: {
		profile: "Profil",
		profileDescription: "Prikaži sebe u najboljem svetlu.",
		blankPage: "Prazna stranica",
		blankPageDescription: "Kreiraj novu pod-stranicu unutar ove.",
		carousel: "Karusel",
		carouselDescription: "Kreriaj niz poveznaih stranica.",
		text: "Tekst",
		textDescription: "Počni sa pisanjem teksta...",
		heading: "Podnaslov",
		headingDescription: "Ubaci podnaslov.",
		title: "Naslov",
		titleDescription: "Ubaci naslov.",
		image: "Slika",
		imageDescription: "Dodaj sliku sa računara ili ubaci link.",
		video: "Reel",
		videoDescription: "Dodaj video sa računara ili ubaci link.",
		audio: "Zvuk",
		audioDescription: "Dodaj audio sa računara ili ubaci link.",
		question: "Pitanje",
		questionDescription: "Testiraj znanje tvoje publike definisanjem pitanja i odgovora.",
		message: "Poruka",
		messageDescription: "Prikaži konverzaciju.",
	},
})

export default localization
